...
static async Task RunDataImportAsync()
{
    ...
    // 询问是否设置文件过滤规则
    Console.WriteLine("是否要设置文件过滤规则？(y/n，默认为n): ");
    string? filterInput = Console.ReadLine();
    FileRecord.Utils.FileFilterRule? filterRule = null;
    
    if (!string.IsNullOrEmpty(filterInput) && filterInput.ToLower().StartsWith("y"))
    {
        Console.WriteLine("选择过滤规则类型:");
        Console.WriteLine("1. 文档文件 (.doc, .pdf, .txt等)");
        Console.WriteLine("2. 图片文件 (.jpg, .png, .gif等)");
        ...
        switch (filterChoice)
        {
            ...
            case "6":
                ...
                if (!string.IsNullOrWhiteSpace(extInput))
                {
                    ...
                    filterRule = FileRecord.Utils.FileFilterRuleFactory.CreateCustomRule(extArray);
